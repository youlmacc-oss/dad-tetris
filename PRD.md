...
이 문서와 `ARCHITECTURE.md`, `UI.md`, `PROMPTS.md`만으로 다른 PC에서 **동일 규칙·동일 ID·동일 저장키**의 게임을 다시 만든다.
...
- `sw.js` 캐시명 `dad-tetris-v1.4.5-mobile`.
...
- [ ] 모바일: 헤더 → 보드+타워 → **7버튼** → **전광판 맨 아래**
- [ ] 창/보드 배경 키 분리, 최고점 키 보호
- [ ] 이벤트 영상 3슬롯 15MB + 슬롯 삭제
- [ ] 12언어, F9 C1–C15, PWA 버전 문자열 일치
