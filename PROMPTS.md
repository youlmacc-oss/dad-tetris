...
버전 문자열: 1.4.4-allpass 를 모든 진입점에 동일하게 쓴다.
...
5. 모바일 하단 순서: 전광판(stage row 3) 위, 7버튼 패드(#game-container 밖) 맨 아래.
6. #mobile-controls 를 main 이나 #game-container 안으로 옮기지 마라.
7. 공장 배경 파일명은 assets/images/default_bg.jpg 와 level_1.jpg–level_10.jpg 만.
8. 오토플레이가 켜진 판은 명예의 전당에 넣지 마라.
9. 대기 카드는 보드 패널 정중앙 flex. 50%+translate 금지.
10. 전광판 PC 높이 160px 고정.
11. 문서에 없는 기능/탭/키를 창작하지 마라. 패드와 전광판을 맞바꾸지 마라.
12. 회전: ArrowUp = 시계(+1), KeyZ = 반시계(-1). ARCHITECTURE §16 킥/전광판/AI/전당 스키마를 그대로 구현하라.

구현 순서:
A. 파일 골격과 DOM ID (UI.md 트리 그대로)
B. 엔진: 7-bag, SRS 표, 점수/중력 공식, 홀드/고스트
C. HUD 데스크톱 350px 사이드바
D. 모바일 100dvh 3행 그리드 + 컨테이너 밖 7버튼
...
#mobile-controls 는 </div><!-- #game-container --> 다음에 둔다.
canvas #bg-canvas #tetris-canvas 는 400×800.
style.css 에 :root 변수, neon-blue, 데스크톱 flex, 모바일 768 3행 그리드를 넣어라.
script.js 는 아직 빈 루프여도 된다. 버전 1.4.4-allpass.
다른 파일/기능을 추가하지 마라.
```
...
ArrowUp 시계, Z 반시계. kicks()는 CCW일 때 오프셋 부호 반전.
...
#game-container 높이 calc(100dvh - 48px).
main.stage 3행: header / board+tower / cheer(grid-row 3).
#mobile-controls 는 container 밖, 높이 48px, 맨 아래.
버튼 7개 ID·색은 UI.md 표.
폭 ≤768 에서 ROWS=20 강제.
패드를 main 그리드에 넣지 마라. PC 레이아웃·JS 규칙을 바꾸지 마라.
```
...
C6은 모바일 20행·100dvh·전광판이 패드보다 위인지 본다.
sw.js 캐시명 dad-tetris-v1.4.4-allpass. localhost는 SW 미등록.
```
...
점수 키, 창/보드 키, 모바일 20행, 전광판↔패드 순서, #mobile-controls DOM 위치,
버전 문자열을 요청에 없으면 건드리지 마라.
js/* 를 살리지 마라.
```
...
#mobile-controls 를 HTML에서 옮기지 마라.
전광판은 stage row 3, 패드는 container 밖 맨 아래를 유지한다.
```
...
전광판/패드 top 좌표와 전광판 높이를 실측하라.
```

기대 실측 (390×844 근처):

- `cheer.bottom <= pad.top + 2`
- `pad` 가 게임 UI 중 가장 아래
- `#mobile-controls.parentElement` 가 `#game-container` 가 아님
...
| 패드를 `main` 안으로 옮김 | `#game-container` 밖 형제 |
| 전광판과 패드 자리 교체 | 전광판 row3, 패드 맨 아래 |
| 배경 한 장만 둠 | 창/보드 이중 키 |
...
| 오토플레이 점수 저장 | 저장 금지 |
| 언어 10개만 (옛 Java PRD) | 12개. 키 없으면 ko 폴백 |
| ↑와 Z를 둘 다 시계 | ↑ 시계, Z 반시계 |
...
마스터 프롬프트의 절대 규칙을 지키며 DAD TETRIS를 이어서 구현해라.
