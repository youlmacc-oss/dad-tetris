...
| ID | primary / accent / bg |
|----|------------------------|
| neon-blue | `#00d2ff` / `#7cf0ff` / `#05070c` |
| cyber-pink | `#ff4fa3` / `#ff7ad9` / `#12060e` |
| emerald-green | `#22f0b2` / `#86efac` / `#050c0a` |
| sunset-orange | `#ff8a3d` / `#ffd76a` / `#0c0805` |
| future-cyber | `#c084fc` / `#facc15` / `#090614` |
...
│               │   └ #dad-cheer-tip
│               └ #controls-guide
├ #mobile-controls          ← #game-container 다음 형제. main 안이 아님
...
| 7 | `.sidebar-dock` | 전광판 **160px 고정** + 조작법 카드 |
...
#game-container { width: 100%; max-width: 480px; height: calc(100dvh - 48px); padding: 4px 6px 0; }
--mobile-header-h: 48px
--mobile-pad-h: 48px
```

`aside.sidebar`, `.sidebar-dock` → `display: contents`  
헤더·보드·타워·전광판이 `main.stage` 그리드 아이템이 된다.  
패드는 그리드에 넣지 않고, 컨테이너 **아래** body flex 아이템으로 48px를 차지한다.
...
main.stage
  ...
  rows:    auto   calc(100dvh - 190px)   minmax(34px, 1fr)
...
| **3** | **1 / -1** | **`#dad-cheer-banner` 가로 슬림 바** |

그 아래, `#game-container` 밖:

| | |
|--|--|
| body 다음 아이템 | `#mobile-controls` 높이 48px, 화면 **맨 하단** |

**이 순서를 바꾸지 말 것.** 패드를 `main` 안으로 옮기거나 전광판과 자리를 맞바꾸면 PC·모바일 전체가 흔들린다.

전광판 모바일: 가로 flex, 뱃지+한 줄 텍스트. `#dad-cheer-tip` 숨김.  
패드: `position: relative`, `flex: 0 0 48px`,  
`padding: 0 2px max(4px, env(safe-area-inset-bottom, 4px)) 2px`.
...
가이드 카피(모바일): 보드 아래 가로 와이드 전광판, **최하단은 7버튼 터치패드**.
...
- 컨테이너 max 480, 높이 `100dvh - 48px`
- 전광판 top < 패드 top (전광판이 위, 패드가 아래)
- 패드가 뷰포트에서 가장 아래 게임 UI
- 타워가 보드 오른쪽
- 헤더 3버튼 보임
